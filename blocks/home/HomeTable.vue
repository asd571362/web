...
import { orc20Columns, orcCashColumns } from '~/utils/table-columns'
...
const getParmas = () => {
  ...
  params = {
    ...
    ...filterParams,
    complete,
    ticker: searchValue.value ? searchValue.value.trim() : undefined
  }
  return params
}
...
const { data, pending, refresh } = useAsyncData(
  ...
  async () => {
    ...
    return await api.orc20TokenInfos(params)
  },
  {
    watch: [pageNo],
    server: false,
...
const rowProps = (row: RowData) => {
  return {
    ...
    onClick: () => {
      $router.push(path + '/token/' + row.tokenInfoInscriptionNumber)
    }
  }
}

const showSearchInput = computed<boolean>(() => {
  return ['orc-20', 'sell', 'orc-cash'].includes(protocol)
})
...
function handleUpdateSorter(sorterInfo: any) {
  ...
  const map: { [key: string]: string } = {
    tokenInfoInscriptionNumber: 'inscription_number',
    tokenInfoProgress: 'progress',
    tokenInfoDeployTimes: 'id',
    tokenInfoHolders: 'holders',
    tokenInfoTransactions: 'transactions'
  }
  sorter.value =
...
function handleUpdateFilters(filterInfo: any) {
  ...
  const upgradable = [...(filterInfo.tokenInfoUpgradable ?? [])]
  filter.value.upgradable =
    (upgradable.includes('yes') && upgradable.includes('no')) ||
    upgradable.length === 0
      ? undefined
      : upgradable.includes('yes')
      ? 'true'
      : 'false'
  const protocol = [...(filterInfo.tokenInfoProtocol ?? [])]
  filter.value.protocol =
    (protocol.includes('orc-cash') &&
      protocol.includes('brc-20') &&
      protocol.includes('orc-20')) ||
    protocol.length === 0
      ? undefined
      : protocol.includes('orc-cash')
      ? 'orc-cash'
